...
extern void bhv_spinning_block();

const BehaviorScript bhvSpinningPlatform[] = {
    BEGIN(OBJ_LIST_SURFACE),
    OR_INT(oFlags, OBJ_FLAG_UPDATE_GFX_POS_AND_ANGLE),
    LOAD_COLLISION_DATA(spinning_platform_collision),
    SET_FLOAT(oDrawingDistance, 20000),
    SET_HOME(),
    BEGIN_LOOP(),
        CALL_NATIVE(bhv_spinning_block),
        CALL_NATIVE(load_object_collision_model),
    END_LOOP(),
};

extern void bhv_warp_custom_loop();

const BehaviorScript bhvWarpCustom[] = {
    BEGIN(OBJ_LIST_LEVEL),
    OR_INT(oFlags, (OBJ_FLAG_SET_FACE_YAW_TO_MOVE_YAW | OBJ_FLAG_UPDATE_GFX_POS_AND_ANGLE)),
    SET_INT(oInteractType, INTERACT_WARP),
    SET_INT(oIntangibleTimer, 0),
    BEGIN_LOOP(),
        CALL_NATIVE(bhv_warp_custom_loop),
    END_LOOP(),
};