...
void update_mario_health(struct MarioState *m) {
    ...
    if (m->health >= 0x100) {
        ...
        if(m->parryTimer == 0 && m->flags & MARIO_PARRY_SUCCESS){
            m->flags &= ~ MARIO_PARRY_TRY;
            m->flags &= ~ MARIO_PARRY_SUCCESS;
        }

        if(m->parryTimer == 0 && m->flags & MARIO_PARRY_FAIL){
            m->flags &= ~ MARIO_PARRY_FAIL;
            m->squishTimer = 10;
        }

        if(m->parryTimer == 0 && m->flags & MARIO_PARRY_TRY && !(m->flags & MARIO_PARRY_SUCCESS)){
            m->flags &= ~ MARIO_PARRY_TRY;
            m->flags &= ~ MARIO_PARRY_SUCCESS;
            m->flags |= MARIO_PARRY_FAIL;
            m->squishTimer = 10;
        ...
        if(m->input & INPUT_B_DOWN && !(m->flags & MARIO_PARRY_FAIL)){
            m->flags |= MARIO_PARRY_TRY;
            m->parryTimer = 30;
        }
        ...
        // print_text_fmt_int(225, 180, "TRY %d", (m->flags & MARIO_PARRY_TRY) != 0);
        // print_text_fmt_int(225, 160, "Suc. %d", (m->flags & MARIO_PARRY_SUCCESS) != 0);
        // print_text_fmt_int(225, 140, "Fail %d", (m->flags & MARIO_PARRY_FAIL) != 0);
        // print_text_fmt_int(225, 120, "P %d", m->parryTimer);

        // print_text(54, 180, "54 180");
        // print_text(54, 100, "54 100");
        // print_text(54, 20, "54 20");
        // print_text(150, 90, "150 90");
        // print_text(100, 10, "100 10");
        // print_text(0, 0, "0 0");

        // print_text_fmt_int(225, 100, "Spd %d", m->forwardVel);
        // print_text_fmt_int(225, 80, "X %d", m->pos[0]);
        // print_text_fmt_int(225, 60, "Y %d", m->pos[1]);
        // print_text_fmt_int(225, 40, "Z %d", m->pos[2]);
        
        // if (m->interactObj){
        //     print_text(20, 200, "Gey Bit");
        //     print_text_fmt_int(20, 180, "%d", m->interactObj->oInteractType);
        // }else{
        //     print_text(20, 180, "Not Gey Bit");
        // }
